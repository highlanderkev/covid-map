    ...
    <v-form ref="form" v-model="valid" @submit.prevent="onSubmit" class="mt-4">
      <v-card variant="flat">
        ...
        <v-card-subtitle class="px-0"
          >Get this Data sent to your phone.</v-card-subtitle
        >
...
const phoneRules = [
  ...
  (v: string) =>
    v.replace(/\D/g, '').length >= 10 || 'Must be a valid 10-digit number',
]
...
const confirmedCasesForSelected = computed(
  () => selectedCovidData.value.confirmed || 0,
)
...
const formattedPhoneNumberE164 = computed(
  () => `+1${phoneNumber.value.replace(/\D/g, '')}`,
)
